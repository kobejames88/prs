package com.perfectchina.bns.service.pin;

public interface PinPosition {
	public final String MEMBER = "MEMBER" ;
	public final String NEW_FIVE_STAR = "NEW_5_STAR" ;
	public final String FIVE_STAR = "5_STAR" ;
	public final String RUBY = "RUBY" ;
	public final String EMERALD = "EMERALD" ;
	public final String DIAMOND = "DIAMOND" ;
	public final String GOLD_DIAMOND = "GOLD_DIA" ;
	public final String DOUBLE_GOLD_DIAMOND = "DOU_GOLD_DIA" ;
	public final String TRIPLE_GOLD_DIAMOND = "TRI_GOLD_DIA" ;

	public final int LEVEL_MEMBER = 1;
	public final int LEVEL_NEW_FIVE_STAR = 6;
	public final int LEVEL_FIVE_STAR = 7;
	public final int LEVEL_RUBY = 8;
	// add others if need
	/*
    MEMBER(1,"MEMBER"),
    ONE_STAR(2,"ONE_STAR"),
    TWO_STAR(3,"TWO_STAR"),
    THREE_STAR(4,"THREE_STAR"),
    FOUR_STAR(5,"FOUR_STAR"),
    NEW_FIVE_STAR(6,"NEW_FIVE_STAR"),
    FIVE_STAR(7,"FIVE_STAR"),
    RUBY(8,"RUBY"),
    EMERALD(9,"EMERALD"),
    DIAMONDS(10,"DIAMONDS"),
    GOLD_DRILL(11,"GOLD_DRILL"),
    DOUBLE_GOLD_DRILL(12,"DOUBLE_GOLD_DRILL"),
    THREE_GOLD_DRILL(13,"THREE_GOLD_DRILL");
	*/
	
}
